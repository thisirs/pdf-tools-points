...
(defun pdf-tools-points-extract (&optional directory)
  (interactive "D")
  (let* ((filename (pdf-view-buffer-file-name))
         (groups (-group-by #'car pdf-tools-points-points-list)))
    (mapc (lambda (e)
            (let ((points (mapcar #'cdr (cdr e)))
                  (page (car e)))
              (unless (zerop (% (length points) 2))
                (error "Nombre de points impair"))
              (mapcar (lambda (e)
                        (shell-command (format "convert -define pdf:use-cropbox=true -density 300 '%s[%d]' -chop %fx%f%%+0+0 -crop %fx%f%%+0+0 +repage %s"
                                               filename
                                               (1- page)
                                               (* 100 (nth 1 e))
                                               (* 100 (nth 2 e))
                                               ...
                                               ;; (* 100 (nth 3 e))
                                               ;; (* 100 (nth 4 e))
                                               (format "crop-%d-%d.pdf" page (car e)))))
                      (-map-indexed
                       (lambda (idx bipoint) (cons idx (-flatten bipoint)))
                       (-partition 2 points)))))
          groups)))
